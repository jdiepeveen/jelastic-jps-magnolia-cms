...
mysql -uroot -p${ROOT_PASS} << END
    CREATE DATABASE IF NOT EXISTS author CHARACTER SET utf8 COLLATE utf8_general_ci;
    ...
    GRANT ALL ON author.* TO '${NEW_USER}'@'::1';
    GRANT ALL ON public.* TO '${NEW_USER}'@'::1';

    CREATE USER '${NEW_USER}'@'${HOST}' IDENTIFIED BY '${NEW_PASS}';
    GRANT ALL ON author.* TO '${NEW_USER}'@'${HOST}';
    GRANT ALL ON public.* TO '${NEW_USER}'@'${HOST}';
